...
int create_data_base(main_node **head,char* word,char *file_name)
{
	...
	{
	    m_prev = m_temp;
	    if(strcmp(m_temp -> word, word)==0) //checking word is present 
	    {
		s_temp = m_temp -> s_link;
		while(s_temp != NULL)
		{
		    s_prev = s_temp;
		    if(strcmp(s_temp -> file_name, file_name) == 0)
		    {
			(s_temp -> word_count)++;
			return SUCCESS;
		    }
        ...
        main_node *m_new = malloc(sizeof(main_node));      //insert the main node and sub node at last
        strcpy(m_new -> word, word);
        m_new -> file_count = 1;
    	m_new -> m_link = NULL;
